...
def main(args):
    ...
    for lang in LANGUAGES:
        print(f'Compute duplicates of {lang}')
        detector = DuplicateDetector(min_num_tokens_per_document=5)
        query = f'SELECT id, tokens FROM snippets WHERE language = "{lang}"'
        df = pd.read_sql_query(query, conn)
        for _, row in tqdm(df.iterrows(), desc='Add files'):
            detector.add_file(row['id'], json.loads(row['tokens']), language=lang)
